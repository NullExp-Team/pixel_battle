// ignore_for_file: directives_ordering

//Packages

export 'package:dev_kit/app_dev_kit.dart';

export 'package:collection/collection.dart';
export 'package:flutter_hooks/flutter_hooks.dart';
export 'package:hooks_riverpod/hooks_riverpod.dart';
export 'package:proxima_logger/proxima_logger.dart';
// ignore: invalid_export_of_internal_element
export 'package:riverpod_annotation/riverpod_annotation.dart';
export 'package:freezed_annotation/freezed_annotation.dart';
export 'package:json_annotation/json_annotation.dart';

export 'package:auto_route/auto_route.dart';
export 'package:package_info_plus/package_info_plus.dart';
export 'package:device_preview/device_preview.dart';
export 'dart:async' show FutureOr;

// Api
// export 'package:api/api.dart';

// Core
export 'package:core/src/network/dio_provider.dart';
export 'package:core/src/network/logger_interceptor.dart';
export 'package:core/src/extensions/config_extension.dart';
export 'package:core/src/extensions/hardcoded_string.dart';
export 'package:dev_kit/src/extensions/ref_cache.dart';
export 'package:core/src/hooks/app_lifecycle_listener_hook.dart';
export 'package:core/src/hooks/confetti_controller_hook.dart';
export 'package:core/src/locales/translations.g.dart';
export 'package:core/src/theme/app_colors.dart';
export 'package:core/src/theme/app_shadows.dart';
export 'package:core/src/theme/app_text_styles.dart';
export 'package:core/src/theme/app_theme.dart';
export 'package:core/src/theme/theme_mode_provider.dart';
export 'package:core/src/utils/analytics/analytics_manager.dart';
export 'package:core/src/utils/analytics/analytics_navigator_observer.dart';
export 'package:core/src/utils/app_formatter/app_formatter.dart';
export 'package:core/src/utils/app_formatter/time_duration.dart';
export 'package:core/src/utils/token_storage/parse_jwt.dart';
export 'package:core/src/utils/token_storage/token_storage.dart';
export 'package:core/src/dev_kit/app_api_wrap_controller.dart';
export 'package:core/src/dev_kit/controller_mixin.dart';
export 'package:core/src/dev_kit/validator_mixin.dart';
export 'package:core/src/utils/crashlytics/crashlytics_manager.dart';
export 'package:core/src/utils/flavor/flavor_manager.dart';
export 'package:core/src/utils/flavor/flavor_type.dart';
export 'package:core/src/utils/logger/log.dart';
export 'package:core/src/utils/logger/logger.dart';
export 'package:core/src/utils/providers/app_links_provider.dart';
export 'package:core/src/utils/providers/app_router_provider.dart';
export 'package:core/src/utils/providers/package_info_provider.dart';
export 'package:core/src/utils/remote_config/remote_config_manager.dart';
export 'package:core/src/utils/toast/toast_manager.dart';
export 'package:core/src/utils/toast/api_error_formatter.dart';
export 'package:core/src/utils/validation/shared_validation.dart';
export 'package:core/src/widgets/app_animated_overlay.dart';
export 'package:core/src/widgets/app_wrapper.dart';
export 'package:core/src/widgets/fade_animated.dart';
export 'package:core/src/widgets/s_text.dart';
export 'package:core/src/widgets/svg_icon.dart';
export 'package:core/src/widgets/work_in_progress_screen.dart';
export 'package:core/src/ui_kit/app_button.dart';
export 'package:core/src/ui_kit/app_text_field.dart';
export 'package:core/src/ui_kit/app_tooltip_icon.dart';
export 'package:core/src/assets/assets.gen.dart';
